...
where every entry takes an equal share of the width. Below the mobile breakpoint the entries
stack and the alignment is dropped.
...
A custom icon replaces the glyph on that row and is shown in every variant:
...
| `--quartz-nav-color`, `--quartz-nav-color-hover`, `--quartz-nav-color-active`, `--quartz-nav-color-trail`         | `--darkgray`, `--secondary`, `--secondary`, `--dark` | Link colors by state                                             |
...
`npm run check` runs the typecheck, the linter, the formatter, 76 tests and the build; CI does
