...
Instanz; das Plugin kann beliebig oft aufgeführt werden. Ein Eintrag ohne `layout`-Block fällt nicht
weg: Quartz stellt ihn an die Vorgabe des Plugins, `left` mit `priority: 10`.
...
| `sort`                 | `"manual" \| "alphabetical" \| "date"`                                                                             | `"manual"`                                                                                             | `manual`: `order`-Liste → `navOrder` → Nummernpräfix → Titel. Ohne diese Angaben alphabetisch. `foldersFirst` gilt vor all dem.                                                                                                                                            |
...
| `stripNumericPrefix`   | `boolean`                                                                                                          | `false`                                                                                                | Eine führende Zahl samt Trennzeichen (`01-`, `01 `, `01_`, `01.`) aus Titeln entfernen. Gilt für jeden Titel, der so beginnt, auch für `title` aus dem Frontmatter (`1.2 – Setup` wird `2 – Setup`); `navTitle` bleibt unberührt.                                          |
...
| `exclude`              | `string[]`                                                                                                         | `[]`                                                                                                   | Globs auf Slugs zum Ausblenden, z. B. `private/**`. Ein Ordnername trifft den ganzen Teilbaum; `docs/*` trifft auch `docs/index` und alles darunter, der Ordner verschwindet. Wer nur `docs/index` ausschließt, behält den Ordner; er verlinkt dann auf seine erste Seite. |
...
| `persistState`         | `boolean`                                                                                                          | `false`                                                                                                | Offene Ordner in `localStorage` merken (nur Akkordeon). Mit `exclusive` gewinnt der Ordner der aktuellen Seite gegen einen gemerkten: Ihn wieder zu öffnen würde den Ordner schließen, in dem die Seite liegt.                                                             |
| `trigger`              | `"click" \| "hover"`                                                                                               | `"click"`                                                                                              | Wie sich `dropdown`, `mega` und `flyout` öffnen. Hover öffnet auch bei Tastaturfokus.                                                                                                                                                                                      |
| `breakpoints`          | `{ mobile, desktop }`                                                                                              | aus `quartz/styles/variables.scss`                                                                     | Breakpoints der Site überschreiben, z. B. `{ mobile: 640px }`. Alle Instanzen teilen ein Stylesheet, also in jedem Eintrag denselben Wert setzen. `desktop` wird angenommen, derzeit aber nicht benutzt.                                                                   |
...
`full`, wobei jeder Eintrag den gleichen Anteil der Breite bekommt. Mit `mobile: accordion` oder
`offcanvas` stapeln sich die Einträge unterhalb des Mobile-Breakpoints und die Ausrichtung entfällt;
mit `mobile: same` bricht die Zeile um.
...
Ein eigenes Icon ersetzt die Glyphe dieser Zeile und wird in jeder Variante gezeigt außer in
`select`, dessen Optionen nur Text enthalten können:
...
| `--quartz-nav-color`, `--quartz-nav-color-hover`, `--quartz-nav-color-active`, `--quartz-nav-color-trail`         | `--darkgray`, `--secondary`, `--secondary`, `--secondary`  | Linkfarben je Zustand                                                        |
...
| `--quartz-nav-weight-active`                                                                                      | `600`                                                      | Schriftstärke der aktiven Seite                                              |
| `--quartz-nav-color-icon`, `--quartz-nav-bg-active`                                                               | `--gray`, `--highlight`                                    | Glyphen und Icons; Hintergrund der aktuellen Seite                           |
| `--quartz-nav-icon-size`, `--quartz-nav-accent-bar`                                                               | `1.1em`, `3px`                                             | Icongröße; Akzentbalken an der aktuellen Seite in den vertikalen Varianten   |
| `--quartz-nav-folder-font`                                                                                        | `var(--quartz-nav-heading-font)`                           | Schrift der Ordnertitel                                                      |
| `--quartz-nav-level-1-size` … `--quartz-nav-level-4-size`                                                         | `0.95rem`, `0.9rem`, `0.875rem`, `0.8rem`                  | Größe der Ordnerzeilen je Ebene (Ebene 4 und tiefer teilen den letzten Wert) |
| `--quartz-nav-level-1-weight` … `--quartz-nav-level-4-weight`                                                     | `700`, `600`, `500`, `500`                                 | Schriftstärke der Ordnerzeilen je Ebene                                      |
| `--quartz-nav-page-size`, `--quartz-nav-page-weight`                                                              | `0.875rem`, `400`                                          | Größenobergrenze und Stärke der Seitenzeilen                                 |
| `--quartz-nav-bar-weight`                                                                                         | `500`                                                      | Stärke der obersten Zeile von `bar`, `dropdown`, `mega`, `tabs`              |
